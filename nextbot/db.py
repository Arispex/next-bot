...
def ensure_default_groups() -> None:
    ...
    try:
        ...
        if guest is None:
            session.add(Group(name="guest", permissions="basic.inventory.self,basic.inventory.user,basic.kick.self,basic.online,basic.progress,basic.search_command,economy.sign,economy.transfer,leaderboard.coins,leaderboard.signin,leaderboard.streak,menu.root,server.list,user.info.self,user.info.user,user.register,user.whitelist.sync", inherits=""))
